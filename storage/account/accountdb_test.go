...
import (
	...
	"github.com/xchain/go-chain/storage/xchaindb"
)

func TestUpdateLeaks(t *testing.T) {
	// Create an empty state database
	db, _ := xchaindb.NewMemDatabase()
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transDb, _ := xchaindb.NewMemDatabase()
	finalDb, _ := xchaindb.NewMemDatabase()
	...
	modify := func(state *AccountDB, addr common.Address, i, tweak byte) {
		...
		//if i%2 == 0 {
		//	state.SetData(addr, common.Hash{i, i, i, 0}, common.Hash{})
		//	state.SetData(addr, common.Hash{i, i, i, tweak}, common.Hash{i, i, i, i, tweak})
		//}
		if i%3 == 0 {
...
func (test *snapshotTest) checkEqual(state, checkstate *AccountDB) error {
	for _, addr := range test.addrs {
		...
		// Check storage.
		//if obj := state.getAccountObject(addr); obj != nil {
		//	state.ForEachStorage(addr, func(key, val common.Hash) bool {
		//		return checkeq("GetData("+key.Hex()+")", val, checkstate.GetData(addr, key))
		//	})
		//	checkstate.ForEachStorage(addr, func(key, checkval common.Hash) bool {
		//		return checkeq("GetData("+key.Hex()+")", state.GetData(addr, key), checkval)
		//	})
		//}
		if err != nil {
			return err
