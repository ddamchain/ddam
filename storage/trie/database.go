...
import (
	...
	"github.com/xchain/go-chain/storage/rlp"
)
...
type NodeDatabase struct {
	diskdb xchaindb.Database // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb xchaindb.Database) *NodeDatabase {
	return &NodeDatabase{
		diskdb:    diskdb,
...
func (db *NodeDatabase) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			...
			if batch.ValueSize() > xchaindb.IdealBatchSize {
...
func (db *NodeDatabase) commit(hash common.Hash, batch xchaindb.Batch) error {
