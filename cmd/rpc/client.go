...
const (
	...
	defaultDialTimeout   = 10 * time.Second // Used when dialing if the global has no deadline
	defaultWriteTimeout  = 10 * time.Second // Used for calls if the global has no deadline
...
// CallContext performs a JSON-RPC call with the given arguments. If the global is
