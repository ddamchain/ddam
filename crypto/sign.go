...
var signBytes = [SignLength]byte{}
var EmptySign = BytesToSign(signBytes[:])
...
func (s *Sign) UnmarshalJSON(bs []byte) error {
	var data []byte
...
func (s *Sign) UnmarshalMsgpack(bs []byte) error {
	var data []byte
...
func (s *Sign) MarshalMsgpack() ([]byte, error) {
	bs := s.Bytes()
...
func HexToSign(s string) (si *Sign) {
	...
	buf, _ := hex.DecodeString(s[len(common.HexPrefix):])
	si = BytesToSign(buf)
	return si
}
